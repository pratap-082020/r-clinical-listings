...
trt01pn=5;
output;
keep usubjid trt01p trt01pn saffl DLTEVLFL HEIGHTBL WEIGHTBL BMIBL;
run;
...
%stat ( var1=HEIGHTBL,var2=%str (Height (cm)),n=1);
%stat ( var1=WEIGHTBL,var2=%str (Weight (Kg)),n=2);
%stat ( var1=BMIBL,var2=%str (BMI (kg/m2)),n=3);

data adsl6;
set HEIGHTBL WEIGHTBL BMIBL;
run;
proc sort;by numb od;run;

data dum;
set adsl6 (keep=numb od);
...
title3 j=c "Table 14.1.8  Subject Demographics (Safety Population)";
footnote1 j=l "E:\ROSHE30730\PROGRAMS\tab4_1.sas";
options orientation=landscape;
ods escapecahr='^';

ods rtf file ="E:\ROSHE30730\OUTPUTS\14_1_8.rtf" style=styles.test;
...
COLUMN  numb od stat  t1 t2 t3 t4 t5;
